...
def get_scaler(name):
    ...
    raise ValueError("Only standard scaler supported for this export script")
...
def generate_scores_feature(X, y_train, num_cols, top3_cols=None, weights=None):
    # If top3_cols/weights passed, use them (inference mode)
    # If not, calculate them (training mode) -- BUT strict calculation creates leakage if done on whole train
    # For the final model export, we want to start from features. 
    # In 02_make_score, we generated 'top3_score' via CV. 
    # For the FINAL model that we save, how do we generate 'top3_score' for a new test instance?
    # We need fixed weights and fixed top3 features derived from the Full Train set (or fold average).
    # Strategy: Train ref model on Full Train -> Get Weights -> Save them -> Use them for transformation.
    ...
    if top3_cols is None or weights is None:
        ...
        # Inference mode logic (handled outside or here?)
        # For export, we just need to return the Transformed X and the Meta info
        scaler_score.fit(X[top3_cols]) # Re-fit on train to ensure consistency
...
def main():
    ...
    # Hardcoded defaults as requested: Dataset B, Standard Scaler
    ...
    with LogTee(log_path):
        ...
        # Handle Categoricals (OneHot)
        X = pd.get_dummies(X, drop_first=True)
        
        # 2. Score Generation Feature
        # Need numerical columns list
        ...
        # We need to save column order to ensure test data matches
        ...
        for m_name in models:
            ...
            if m_name == 'rf':
                ...
                clf = XGBClassifier(random_state=42, n_jobs=-1, eval_metric='logloss', **params)
            else:
                clf = LGBMClassifier(random_state=42, verbose=-1, n_jobs=-1, **params)
            
            # 4a. Generate OOF for Threshold Optimization
            # Note: We must be careful. If we use the 'X' which has 'top3_score' generated on full train,
            # we are leaking info into the OOF. 
            # Ideally, to get a ROBUST threshold, we should generate OOF exactly as we did in tuning (score inside fold).
            # BUT, for the Deployment Model, we are training on Full Train. 
            # The discrepancy between "Score generated on Full Train" vs "Score on Test" is minimized if dist is same.
            # For OOF Threshold, we simply cross_val_predict on the transformed X?
            # It's slight leakage but acceptable for threshold calibration on the final model export step.
            
            # Actually, let's just train the final model on Full Data and save it.
            clf.fit(X_scaled, y)
            joblib.dump(clf, f"{out_dir}/{m_name}.pkl")
            model_paths[m_name] = f"{out_dir}/{m_name}.pkl"
            
            # Generate OOF Probs for Ensemble Threshold
            # reusing the same CV split
            oof_p = np.zeros(len(X))
            for tr_idx, val_idx in kf.split(X_scaled, y):
                # We reuse the same model structure but fit on folds
                # We technically should re-fit scaler on folds too but...
                # Let's keep it simple for this export script.
        ...
        # Average OOF of all available models
        if oof_preds:
